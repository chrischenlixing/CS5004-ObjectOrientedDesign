...
public class Sanctuary {
    ...
    public List<String> getEnclosureList() {
        ...
        for (Enclosure enclosure : enclosures) {
            ...
            for (int i = 0; i < monkeys.length; i++) {
                if (monkeys[i] != null && monkeys[i].getSpecies() == enclosure.getSpecies()) {
    ...
    public List<String> getMonkeyinIsolation() {
        List<String> result = new ArrayList<>();
        StringBuilder sb1 = new StringBuilder();

        Monkey[] monkeysInIso = isolation.getMonkeys();
        for (int i = 0; i < monkeysInIso.length; i++) {
            if (monkeysInIso[i] != null) {
                sb1.append(i + ":" + monkeysInIso[i].getName() + ", ");
            }
        }
     ...
     * @return A list of monkeys sorted alphabetically by name.
     */
    public List<String> getAlphabeticalList() {
        List<String> monkeyNames = new ArrayList<>();
        for (Monkey monkey :Enclosure.getMonkeys()) {
            if (monkey != null) {
                monkeyNames.add(monkey.getName());
            }
        }

        for (Monkey monkey : isolation.getMonkeys()) {
            if (monkey != null) {
                monkeyNames.add(monkey.getName());
            }
        }
        Collections.sort(monkeyNames);
        return monkeyNames;
    }
}
